...
public interface ChuongTrinhLienKetApi {
    Gson gson = new GsonBuilder()
            .setDateFormat("")
            .create();
    // Khoi tao retrofit

    ChuongTrinhLienKetApi chuongTrinhLienKetApi = new Retrofit.Builder()
            //.baseUrl("http://192.168.1.6:8080/DoAnTotNghiep/")
            .baseUrl("http://192.168.100.3:8080/DoAnTotNghiep/")
            .addConverterFactory(GsonConverterFactory.create(gson))
            .build()
            .create(ChuongTrinhLienKetApi.class);

    // Call api
    @GET("rest/chuongtrinhlienketapi")
    Call<List<CTLKDoanhNghiep>> allDeXuat();
